...
public class NavitationLayout extends RelativeLayout {
	...
	public void setOnNaPageChangeListener(
			OnNaPageChangeListener onNaPageChangeListener) {
		this.onNaPageChangeListener = onNaPageChangeListener;
	}

	private void setTitles(Context context, String[] titles,
	...
	private void setTitles(Context context, String[] titles,
			final boolean smoothScroll, int splilinecolor, float splilinewidth,
		...
		LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(dip2px(
				context, splilinewidth), LayoutParams.MATCH_PARENT);
		lp.setMargins(0, dip2px(context, topoffset), 0,
				dip2px(context, bottomoffset));
		// 循环，根据标题栏动态生成TextView来显示标题，每个标题栏的宽度比例为1:1,其中的内容居中。
		for (int i = 0; i < titles.length; i++) {
			final int index = i;
			TextView textView = new TextView(context);
	...
	public void setNavLine(Activity context, int height, int color,
		...
		navLine.setBackgroundResource(color);
	 ...
	 * @param setectedcolor
	 *            选中字体颜色
	 * @param txtUnselectedSize
	 *            未选中字体大小
	 * @param txtSelectedSize
	 *            选中字体大小
	 * @param currentPosition
	 *            当前viewpager的位置
	 * @param widOffset
	...
	public void setViewPager(final Context context, String[] titles,
			ViewPager viewPager, final int unselectedcolor,
			final int setectedcolor, int txtUnselectedSize,
			final int txtSelectedSize, final int currentPosition,
			int widOffset, boolean smoothScroll) {
		...
		setSelectedTxtColor(context, setectedcolor, txtSelectedSize,
				currentPosition);
		viewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
			...
			public void onPageScrolled(int position, float positionOffset,
				...
				if (onNaPageChangeListener != null) {
					onNaPageChangeListener.onPageScrolled(position,
	...
	public void setViewPager(final Context context, String[] titles,
			...
			final int setectedcolor, int txtUnselectedSize,
			final int txtSelectedSize, final int currentPosition,
			int widOffset, boolean smoothScroll, int splilinecolor,
		...
		viewPager.setCurrentItem(currentPosition);
		setTitles(context, titles, smoothScroll, splilinecolor, splilinewidth,
	...
	private void moveBar(View bar, int width, float percent, int position) {
		LayoutParams lp = (LayoutParams) bar
				.getLayoutParams();
	...
	private void setSelectedTxtColor(Context context, int selectedcolor,
			int selectedsize, int position) {
		if (textViews != null) {
			int length = textViews.length;
			for (int i = 0; i < length; i++) {
				if (i == position) {
